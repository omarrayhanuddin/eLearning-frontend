...
const CourseTabs = ({ activeTab, handleChangeTab, isAuthenticated, modules, courseId, has_enrolled, isBlocked, instructor }) => {
  const { user } = useAuth();
  const tabs = [ // Array to hold tab configurations
  ...
  return (
    <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
      <Tabs value={activeTab} onChange={handleChangeTab} aria-label="course tabs" centered>
        {tabs.map((tab, index) => (
          <Tab key={index} label={tab.label} /> // Use map for rendering tabs
        ))}
      </Tabs>
      <Box sx={{ mt: 2, maxHeight: 'calc(100vh - 200px)', overflowY: 'auto' }}>
                {tabs[activeTab]?.component} {/* Render component based on activeTab - ONLY THIS IS NEEDED */}
                {/* REMOVE THIS LINE: {activeTab === tabs.findIndex(tab => tab.label === "Live Chat") && <LiveChat courseId={courseId} />} */}
            </Box>
    </Box>
  );
};
